...
impl<'a> DtoRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_use_case_dto_in(event_buffer, snap_store, &snap.root_ids)?;

        // Surgically reconcile the root ids' placement in their (external) strong owner,
        // preserving sibling edits made on other undo stacks.
        self.reconcile_backref_use_case_dto_out(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("dto", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.dtos);
            for id in &ids {
                match (snap.dtos.contains_key(id), live.contains_key(id)) {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_dto_field_from_dto_fields.get(id) {
                    child_ids.extend(list.iter().copied());
                }
            }
            {
                let live_jn = read_or_recover(&store.jn_dto_field_from_dto_fields);
                for id in &ids {
        ...
        {
            let mut live = write_or_recover(&store.dtos);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.dtos.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_dto_field_from_dto_fields);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_dto_field_from_dto_fields.get(id) {
                    Some(v) => {
    ...
    /// `use_case.dto_in`.
    fn reconcile_backref_use_case_dto_in(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_dto_from_use_case_dto_in.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_dto_from_use_case_dto_in);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_dto_from_use_case_dto_in).insert(left, reconciled);
                event_buffer.push(Event {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `use_case.dto_out`.
    fn reconcile_backref_use_case_dto_out(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_dto_from_use_case_dto_out.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_dto_from_use_case_dto_out);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_dto_from_use_case_dto_out).insert(left, reconciled);
                event_buffer.push(Event {
                    origin: Origin::DirectAccess(DirectAccessEntity::UseCase(EntityEvent::Updated)),
                    ids: vec![left],
                    data: None,

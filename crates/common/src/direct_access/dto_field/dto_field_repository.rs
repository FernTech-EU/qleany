...
impl<'a> DtoFieldRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_dto_fields(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("dto_field", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.dto_fields);
            for id in &ids {
                match (snap.dto_fields.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.dto_fields);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.dto_fields.get(id) {
    ...
    /// `dto.fields`.
    fn reconcile_backref_dto_fields(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_dto_field_from_dto_fields.iter() {
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_dto_field_from_dto_fields).insert(left, reconciled);
                event_buffer.push(Event {

...
impl<'a> FeatureRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_workspace_features(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("feature", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.features);
            for id in &ids {
                match (snap.features.contains_key(id), live.contains_key(id)) {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_use_case_from_feature_use_cases.get(id) {
        ...
        {
            let mut live = write_or_recover(&store.features);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.features.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_use_case_from_feature_use_cases);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_use_case_from_feature_use_cases.get(id) {
                    Some(v) => {
        ...
        self.reconcile_backref_file_feature(event_buffer, snap, &ids)?;
    ...
    /// `file.feature`.
    fn reconcile_backref_file_feature(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_feature_from_file_feature.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_feature_from_file_feature);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_feature_from_file_feature).insert(left, reconciled);
                event_buffer.push(Event {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `workspace.features`.
    fn reconcile_backref_workspace_features(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_feature_from_workspace_features.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_feature_from_workspace_features)

...
impl<'a> FieldRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_entity_fields(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("field", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.fields);
            for id in &ids {
                match (snap.fields.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.fields);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.fields.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_entity_from_field_entity);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_entity_from_field_entity.get(id) {
                    Some(v) => {
        ...
        self.reconcile_backref_file_field(event_buffer, snap, &ids)?;
    ...
    /// `file.field`.
    fn reconcile_backref_file_field(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_field_from_file_field.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_field_from_file_field);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_field_from_file_field).insert(left, reconciled);
                event_buffer.push(Event {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `entity.fields`.
    fn reconcile_backref_entity_fields(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_field_from_entity_fields.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_field_from_entity_fields).insert(left, reconciled);
                event_buffer.push(Event {

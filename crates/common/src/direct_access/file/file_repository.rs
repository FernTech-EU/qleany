...
impl<'a> FileRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_system_files(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("file", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.files);
            for id in &ids {
                match (snap.files.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.files);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.files.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_entity_from_file_entity);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_entity_from_file_entity.get(id) {
                    Some(v) => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_feature_from_file_feature);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_feature_from_file_feature.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_field_from_file_field);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_field_from_file_field.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_use_case_from_file_use_case);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_use_case_from_file_use_case.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
    ...
    /// `system.files`.
    fn reconcile_backref_system_files(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_file_from_system_files.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_file_from_system_files);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_file_from_system_files).insert(left, reconciled);
                event_buffer.push(Event {

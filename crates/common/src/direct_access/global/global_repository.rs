...
impl<'a> GlobalRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_workspace_global(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("global", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.globals);
            for id in &ids {
                match (snap.globals.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.globals);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.globals.get(id) {
    ...
    /// `workspace.global`.
    fn reconcile_backref_workspace_global(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_global_from_workspace_global.iter() {
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_global_from_workspace_global).insert(left, reconciled);
                event_buffer.push(Event {

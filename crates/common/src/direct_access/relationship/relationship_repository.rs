...
impl<'a> RelationshipRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_entity_relationships(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("relationship", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.relationships);
            for id in &ids {
                match (snap.relationships.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.relationships);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.relationships.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_entity_from_relationship_left_entity);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_entity_from_relationship_left_entity.get(id) {
                    Some(v) => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_entity_from_relationship_right_entity);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_entity_from_relationship_right_entity.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
    ...
    fn reconcile_backref_entity_relationships(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_relationship_from_entity_relationships.iter() {
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_relationship_from_entity_relationships)

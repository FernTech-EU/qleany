    ...
    pub(crate) fn restore_subtree(
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_workspace_from_root_workspace.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_workspace_from_root_workspace);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_workspace_from_root_workspace.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {

...
impl<'a> SystemRepository<'a> {
    ...
    pub(crate) fn restore_subtree(
        ...
        {
            let live = read_or_recover(&store.systems);
            for id in &ids {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_file_from_system_files.get(id) {
        ...
        {
            let mut live = write_or_recover(&store.systems);
            for id in to_create.iter().chain(to_update.iter()) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_file_from_system_files);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_file_from_system_files.get(id) {
                    Some(v) => {
    ...
    fn reconcile_backref_root_system(
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_system_from_root_system).insert(left, reconciled);
                event_buffer.push(Event {

...
impl<'a> UseCaseRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_feature_use_cases(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("use_case", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.use_cases);
            for id in &ids {
                match (snap.use_cases.contains_key(id), live.contains_key(id)) {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_dto_from_use_case_dto_in.get(id) {
        ...
        }
        {
            let mut child_ids: Vec<EntityId> = Vec::new();
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_dto_from_use_case_dto_out.get(id) {
                    child_ids.extend(list.iter().copied());
                }
            }
            {
        ...
        {
            let mut live = write_or_recover(&store.use_cases);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.use_cases.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_dto_from_use_case_dto_in);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_dto_from_use_case_dto_in.get(id) {
                    Some(v) => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_dto_from_use_case_dto_out);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_dto_from_use_case_dto_out.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_entity_from_use_case_entities);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_entity_from_use_case_entities.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        self.reconcile_backref_file_use_case(event_buffer, snap, &ids)?;
    ...
    /// `file.use_case`.
    fn reconcile_backref_file_use_case(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_use_case_from_file_use_case.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_use_case_from_file_use_case);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_use_case_from_file_use_case).insert(left, reconciled);
                event_buffer.push(Event {
    ...
    /// Surgically reconcile one external referrer junction (out-of-scope key -> in-scope values):
    /// `feature.use_cases`.
    fn reconcile_backref_feature_use_cases(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        // External left keys whose ordered list references any scope id, in snapshot or live.
        let mut left_keys: std::collections::HashSet<EntityId> = std::collections::HashSet::new();
        for (left, rights) in snap.jn_use_case_from_feature_use_cases.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_use_case_from_feature_use_cases);
            for (left, rights) in live_jn.iter() {
                if rights.iter().any(|rid| scope.contains(rid)) {
                    left_keys.insert(*left);
                }
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_use_case_from_feature_use_cases)

...
impl<'a> UseCaseTable for UseCaseHashMapTable<'a> {
    ...
    fn create_multi(&mut self, entities: &[UseCase]) -> Result<Vec<UseCase>, RepositoryError> {
        ...
        let mut use_case_map = write_or_recover(&self.store.use_cases);
        ...
        for entity in entities {
            ...
            {
                let jn = read_or_recover(&self.store.jn_dto_from_use_case_dto_in);
                for (&existing_id, right_ids) in jn.iter() {
                    // A let-chain rather than nested `if`s: edition 2024 has them, and
                    // clippy::collapsible_if flags the nested form.
            ...
            {
                let jn = read_or_recover(&self.store.jn_dto_from_use_case_dto_out);
                for (&existing_id, right_ids) in jn.iter() {
                    // A let-chain rather than nested `if`s: edition 2024 has them, and
                    // clippy::collapsible_if flags the nested form.
    ...
    fn get(&self, id: &EntityId) -> Result<Option<UseCase>, RepositoryError> {
        let use_case_map = read_or_recover(&self.store.use_cases);

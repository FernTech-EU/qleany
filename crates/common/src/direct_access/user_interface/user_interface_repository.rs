...
impl<'a> UserInterfaceRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_workspace_user_interface(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("user_interface", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.user_interfaces);
            for id in &ids {
                match (snap.user_interfaces.contains_key(id), live.contains_key(id)) {
        ...
        {
            let mut live = write_or_recover(&store.user_interfaces);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.user_interfaces.get(id) {
    ...
    fn reconcile_backref_workspace_user_interface(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_user_interface_from_workspace_user_interface.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_user_interface_from_workspace_user_interface);

...
impl<'a> WorkspaceRepository<'a> {
    ...
    pub fn restore(
        ...
        self.reconcile_backref_root_workspace(event_buffer, snap_store, &snap.root_ids)?;
    ...
    pub(crate) fn restore_subtree(
        ...
        let ids: Vec<EntityId> = ids
            .iter()
            .copied()
            .filter(|id| visited.insert(("workspace", *id)))
            .collect();
        if ids.is_empty() {
            return Ok(());
        }
        ...
        {
            let live = read_or_recover(&store.workspaces);
            for id in &ids {
                match (snap.workspaces.contains_key(id), live.contains_key(id)) {
        ...
        //    all locks before recursing so self-referential ownership cannot deadlock).
        {
            ...
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_global_from_workspace_global.get(id) {
                    child_ids.extend(list.iter().copied());
                }
            }
            {
                let live_jn = read_or_recover(&store.jn_global_from_workspace_global);
                for id in &ids {
        ...
        }
        {
            let mut child_ids: Vec<EntityId> = Vec::new();
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_entity_from_workspace_entities.get(id) {
                    child_ids.extend(list.iter().copied());
                }
            }
            {
        ...
        }
        {
            let mut child_ids: Vec<EntityId> = Vec::new();
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_feature_from_workspace_features.get(id) {
                    child_ids.extend(list.iter().copied());
                }
            }
            {
                let live_jn = read_or_recover(&store.jn_feature_from_workspace_features);
                for id in &ids {
                    if let Some(list) = live_jn.get(id) {
                        child_ids.extend(list.iter().copied());
                    }
        ...
        }
        {
            let mut child_ids: Vec<EntityId> = Vec::new();
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(list) = snap.jn_user_interface_from_workspace_user_interface.get(id) {
                    child_ids.extend(list.iter().copied());
                }
            }
            {
                let live_jn =
                    read_or_recover(&store.jn_user_interface_from_workspace_user_interface);
                for id in &ids {
                    if let Some(list) = live_jn.get(id) {
                        child_ids.extend(list.iter().copied());
                    }
        ...
        {
            let mut live = write_or_recover(&store.workspaces);
            for id in to_create.iter().chain(to_update.iter()) {
                if let Some(row) = snap.workspaces.get(id) {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_entity_from_workspace_entities);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_entity_from_workspace_entities.get(id) {
                    Some(v) => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_feature_from_workspace_features);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_feature_from_workspace_features.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        {
            let mut live_jn = write_or_recover(&store.jn_global_from_workspace_global);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_global_from_workspace_global.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
        ...
        {
            let mut live_jn =
                write_or_recover(&store.jn_user_interface_from_workspace_user_interface);
            for id in to_create.iter().chain(to_update.iter()) {
                match snap.jn_user_interface_from_workspace_user_interface.get(id) {
                    Some(v) => {
                        live_jn.insert(*id, v.clone());
                    }
                    None => {
    ...
    /// `root.workspace`.
    fn reconcile_backref_root_workspace(
        &self,
        event_buffer: &mut EventBuffer,
        snap: &HashMapStoreSnapshot,
        scope_ids: &[EntityId],
        ...
        for (left, rights) in snap.jn_workspace_from_root_workspace.iter() {
            if rights.iter().any(|rid| scope.contains(rid)) {
                left_keys.insert(*left);
            }
        }
        {
            let live_jn = read_or_recover(&store.jn_workspace_from_root_workspace);
        ...
        for left in left_keys {
            ...
            if let Some(reconciled) = new_list {
                write_or_recover(&store.jn_workspace_from_root_workspace).insert(left, reconciled);
                event_buffer.push(Event {

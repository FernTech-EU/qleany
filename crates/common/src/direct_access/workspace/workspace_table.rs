...
impl<'a> WorkspaceTable for WorkspaceHashMapTable<'a> {
    ...
    fn create_multi(&mut self, entities: &[Workspace]) -> Result<Vec<Workspace>, RepositoryError> {
        ...
        let mut workspace_map = write_or_recover(&self.store.workspaces);
        ...
        for entity in entities {
            ...
            {
                let jn = read_or_recover(&self.store.jn_global_from_workspace_global);
            ...
            {
                let jn =
                    read_or_recover(&self.store.jn_user_interface_from_workspace_user_interface);
                for (&existing_id, right_ids) in jn.iter() {
    ...
    fn get(&self, id: &EntityId) -> Result<Option<Workspace>, RepositoryError> {
        let workspace_map = read_or_recover(&self.store.workspaces);
